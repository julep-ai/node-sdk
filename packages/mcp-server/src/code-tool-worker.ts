...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Julep)`
    : `const run: (${functionSource.client}: Julep) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.agents.tools.reset',
    'client.agents.tools.update',
    'client.agents.docs.bulkDelete',
    ...
    'client.sessions.render',
    'client.sessions.reset',
