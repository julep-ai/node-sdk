...
import { ClientOptions } from '@julep/sdk';

const prompt = `Runs JavaScript code to interact with the Julep API.
...
async function run(client) {
  const task = await client.tasks.create('dad00000-0000-4000-a000-000000000000', { main: [{ evaluate: { foo: {} } }], name: 'x' });
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    JULEP_API_KEY: requireValue(
      readEnv('JULEP_API_KEY') ?? client.apiKey,
      'set JULEP_API_KEY environment variable or provide apiKey client option',
    ),
    JULEP_BASE_URL:
      readEnv('JULEP_BASE_URL') ?? readEnv('JULEP_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
  };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      ...
      code,
      intent,
      client_opts: { environment: (readEnv('JULEP_ENVIRONMENT') || undefined) as any },
