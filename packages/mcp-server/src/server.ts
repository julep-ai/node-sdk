...
export { ClientOptions } from '@julep/sdk';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/julep',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the julep MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  const client = new Julep({
    ...{ environment: (readEnv('JULEP_ENVIRONMENT') || undefined) as any },
